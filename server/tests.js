...
const rowToTest = (r) => r ? { id: r.id, title: r.title, description: r.description,
  ...
  durationMin: r.duration_min == null ? 0 : r.duration_min, createdAt: r.created_at } : null;
...
function create({ title, description, questionIds, batchIds, durationMin }) {
  title = (title || '').trim();
  ...
  const t = { id: 't' + Date.now().toString(36) + Math.random().toString(36).slice(2,6), title, description: (description || '').trim(),
    ...
    durationMin: Math.max(0, parseInt(durationMin, 10) || 0), createdAt: Date.now() };
  db.prepare('INSERT INTO tests (id,title,description,question_ids,batch_ids,duration_min,created_at) VALUES (?,?,?,?,?,?,?)')
    .run(t.id, t.title, t.description, J(t.questionIds), J(t.batchIds), t.durationMin, t.createdAt);
  return t;
}
