...
function create({ title, description, questionIds, batchIds, durationMin, availability, startAt, openHours, showScore, showAnswers, showSolutions }) {
  title = (title || '').trim();
  ...
  const t = { id: 't' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6), title, description: (description || '').trim(),
    questionIds: Array.isArray(questionIds) ? questionIds : [], batchIds: Array.isArray(batchIds) ? batchIds : [],
    ...
    showAnswers: showAnswers ? 1 : 0, showSolutions: showSolutions ? 1 : 0, createdAt: Date.now() };
  db.prepare(`INSERT INTO tests (id,title,description,question_ids,batch_ids,duration_min,availability,start_at,open_hours,show_score,show_answers,show_solutions,created_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .run(t.id, t.title, t.description, J(t.questionIds), J(t.batchIds), t.durationMin,
      t.availability, t.startAt, t.openHours, t.showScore, t.showAnswers, t.showSolutions, t.createdAt);
  return t;
}
...
module.exports = { list, getById, create, remove, forBatch, windowStatus };
